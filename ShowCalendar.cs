...
namespace Calendar
{
    internal class ShowCalendar
    {
        ...
        GetDateInfo dateInfo = new GetDateInfo();
        private User userInfo;
        private int month;
        private int year;
        private int chosenDay;
        private KeyActions key;
        private AppointmentManager appointment;
        public ShowCalendar(User user)
        {
            this.chosenDay = 1;
            month = dateInfo.CurrentMonthNumber();
            year = dateInfo.CurrentYearNumber();
            userInfo = user;
            this.key = new KeyActions(chosenDay, month, year);
            key.setCal(Calendar);
        ...
        private void Calendar(int month, int year, int chosenDay)
        {
            ...
            for (int i = 1; i <= dateInfo.GetMonthDays(month, year); i++)
            {
